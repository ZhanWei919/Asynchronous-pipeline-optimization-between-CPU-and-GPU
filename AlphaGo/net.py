...
import torchviz
import torch
from data_loader import pcie_counter
...
class Policy(nn.Module):
    def __init__(self, ind=RES_BLOCK_FILLTERS, outd=OUTD, kernels=2):
        ...
        self.linear = nn.Linear(kernels * outd, outd)  # 输出维度为SIZE**2（如64）

    def forward(self, x):
        x = self.conv(x)
        x = x.view(-1, self.out)  # 形状：(batch_size, 64)
        x = self.linear(x)
        return F.log_softmax(x, dim=-1)  # 输出形状：(batch_size, 64)
